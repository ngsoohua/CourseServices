...
public class Main
{
        ...
        throws ParseException
    {
        ...
        do
        {
            try{
                ...
                //Just to prevent entering of text and encountering error.
                // Since not in requirement didnt add the printout for error messages
    ...
    private static void gradeStudent( StudentService studentService, Scanner scanner )
    {
        ...
        if(student != null){
            ...
            HashMap <String, EnrolledCourse> eCourseList = student.getEnrolledCourses();
            if(eCourseList.size() > 0){

                for(EnrolledCourse e : eCourseList.values()){
                ...
                Course eCourse = studentService.findEnrolledCourse(student,courseId);

                System.out.println(String.format("Insert course grade for : %s",eCourse.getName()));
                double courseGrade = scanner.nextDouble();

                studentService.gradeStudent(student,eCourse,courseGrade);
            }else{
                System.out.println("There is no Enrolled Course");
    ...
    private static void showPassedCourses(StudentService studentService, Scanner scanner )
    {
        ...
        Student student = studentService.findStudent(studentId);
        if(student != null){
            HashMap<String, EnrolledCourse> passedCoursesList =  student.findPassedCourses();
            if(passedCoursesList.size()>0){
                for(EnrolledCourse e : passedCoursesList.values()){
                    System.out.println(e);
                }
            }else{
                System.out.println("No Course Enrolled or Student didn't pass any course");
            }
